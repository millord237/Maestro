...
export class ProcessManager extends EventEmitter {
	...
	spawn(config: ProcessConfig): { pid: number; success: boolean } {
		...
		if (hasImages && prompt && capabilities.supportsStreamJsonInput) {
			...
			logger.debug(
				'[ProcessManager] Using file-based image args',
				'ProcessManager',
				{
					sessionId,
		...
		const spawnConfigLogFn = isWindows
		...
		try {
			if (usePty) {
				...
				if (isTerminal) {
					...
					if (shellEnvVars && Object.keys(shellEnvVars).length > 0) {
						...
						for (const [key, value] of Object.entries(shellEnvVars)) {
							...
							ptyEnv[key] = value.startsWith('~/')
								? path.join(homeDir, value.slice(2))
								: value;
						}
						logger.debug(
							'Applied custom shell env vars to PTY',
							'ProcessManager',
							{
				...
				ptyProcess.onData(data => {
					...
					const cleanedData = stripControlSequences(
						data,
						managedProc?.lastCommand,
						isTerminal
					);
					...
					if (cleanedData.trim()) {
						this.emitDataBuffered(sessionId, cleanedData);
					}
				});
				...
				const isResuming =
					finalArgs.includes('--resume') || finalArgs.includes('--session');
				if (isResuming) {
				...
				if (customEnvVars && Object.keys(customEnvVars).length > 0) {
					...
					logger.debug(
						'[ProcessManager] Applied custom env vars',
						'ProcessManager',
						{
							sessionId,
				...
				logger.debug(
					'[ProcessManager] About to spawn child process',
					'ProcessManager',
					{
						command,
						finalArgs,
				...
				if (isWindows) {
					...
					if (useShell) {
						...
						logger.info(
							'[ProcessManager] Escaped args for Windows shell',
							'ProcessManager',
							{
								...
								escapedPromptArgPreview: spawnArgs[
									spawnArgs.length - 1
								]?.substring(0, 200),
				...
				const spawnLogFn = isWindows
					? logger.info.bind(logger)
					: logger.debug.bind(logger);
				spawnLogFn(
					'[ProcessManager] About to spawn with shell option',
					'ProcessManager',
					{
						sessionId,
						spawnCommand,
						...
						promptArgLength: prompt
							? spawnArgs[spawnArgs.length - 1]?.length
							: undefined,
						// Log the full command that will be executed (for debugging)
						fullCommandPreview: `${spawnCommand} ${spawnArgs
							.slice(0, 5)
							.join(' ')}${spawnArgs.length > 5 ? ' ...' : ''}`
					}
				);
				...
				const argsContain = (pattern: string) =>
					finalArgs.some(arg => arg.includes(pattern));
				...
				logger.debug(
					'[ProcessManager] Output parser lookup',
					'ProcessManager',
					{
						sessionId,
						toolType,
						...
						argsPreview:
							finalArgs.length > 0
								? finalArgs[finalArgs.length - 1]?.substring(0, 200)
								: undefined
					}
				);
				...
				const managedProcess: ManagedProcess = {
					...
					tempImageFiles:
						tempImageFiles.length > 0 ? tempImageFiles : undefined, // Temp files to clean up on exit
					command,
					args: finalArgs,
				...
				logger.debug(
					'[ProcessManager] Setting up stdout/stderr/exit handlers',
					'ProcessManager',
					{
						sessionId,
				...
				if (childProcess.stdout) {
					logger.debug(
						'[ProcessManager] Attaching stdout data listener',
						'ProcessManager',
						{ sessionId }
					);
					...
					childProcess.stdout.on('data', (data: Buffer | string) => {
						...
						if (sessionId.includes('group-chat-')) {
							...
							console.log(
								`[GroupChat:Debug:ProcessManager] Raw output length: ${output.length}`
							);
							console.log(
								`[GroupChat:Debug:ProcessManager] Raw output preview: "${output.substring(
									0,
									500
								)}${output.length > 500 ? '...' : ''}"`
							);
						...
						if (isStreamJsonMode) {
							...
							for (const line of lines) {
								...
								if (
									...
									if (sshError) {
										...
										logger.debug(
											'[ProcessManager] SSH error detected from output',
											'ProcessManager',
											{
												sessionId,
								...
								try {
									...
									if (outputParser) {
										...
										logger.debug(
											'[ProcessManager] Parsed event from output parser',
											'ProcessManager',
											{
												sessionId,
										...
										if (event) {
											...
											if (eventSessionId && !managedProcess.sessionIdEmitted) {
												...
												logger.debug(
													'[ProcessManager] Emitting session-id event',
													'ProcessManager',
													{
														sessionId,
														eventSessionId,
											...
											const slashCommands =
												outputParser.extractSlashCommands(event);
											if (slashCommands) {
											...
											if (event.type === 'text') {
												logger.debug(
													'[ProcessManager] Checking thinking-chunk conditions',
													'ProcessManager',
													{
														sessionId,
														eventType: event.type,
											...
											if (
												...
												logger.debug(
													'[ProcessManager] Emitting thinking-chunk',
													'ProcessManager',
													{
														sessionId,
														textLength: event.text.length
											...
											if (
												...
												const resultText =
													event.text || managedProcess.streamedText || '';
												...
												if (sessionId.includes('-synopsis-')) {
													logger.info(
														...
														'ProcessManager',
														{
															sessionId,
															eventText:
																event.text?.substring(0, 200) || '(empty)',
															eventTextLength: event.text?.length || 0,
															streamedText:
																managedProcess.streamedText?.substring(
																	0,
																	200
																) || '(empty)',
															streamedTextLength:
																managedProcess.streamedText?.length || 0,
												...
												if (resultText) {
													logger.debug(
														'[ProcessManager] Emitting result data via parser',
														'ProcessManager',
														{
															sessionId,
													...
													this.emitDataBuffered(sessionId, resultText);
										...
										if (
											...
											managedProcess.resultEmitted = true;
											logger.debug(
												'[ProcessManager] Emitting result data',
												'ProcessManager',
												{ sessionId, resultLength: msg.result.length }
											);
											this.emitDataBuffered(sessionId, msg.result);
										}
									...
									this.emitDataBuffered(sessionId, line);
								}
							}
						} else if (isBatchMode) {
							...
							managedProcess.jsonBuffer =
								(managedProcess.jsonBuffer || '') + output;
							logger.debug(
								'[ProcessManager] Accumulated JSON buffer',
								'ProcessManager',
								{ sessionId, bufferLength: managedProcess.jsonBuffer.length }
							);
						} else {
							// In interactive mode, emit data immediately
							this.emitDataBuffered(sessionId, output);
						}
					});
				} else {
					logger.warn(
				...
				if (childProcess.stderr) {
					logger.debug(
						'[ProcessManager] Attaching stderr data listener',
						'ProcessManager',
						{ sessionId }
					);
					...
					childProcess.stderr.on('data', (data: Buffer | string) => {
						...
						logger.debug(
							'[ProcessManager] stderr event fired',
							'ProcessManager',
							{ sessionId, dataPreview: stderrData.substring(0, 100) }
						);
						...
						if (sessionId.includes('group-chat-')) {
							...
							console.log(
								`[GroupChat:Debug:ProcessManager] Stderr length: ${stderrData.length}`
							);
							console.log(
								`[GroupChat:Debug:ProcessManager] Stderr preview: "${stderrData.substring(
									0,
									500
						...
						managedProcess.stderrBuffer = appendToBuffer(
						...
						if (cleanedStderr) {
							...
							const isKnownSshInfo = sshInfoPatterns.some(pattern =>
								pattern.test(cleanedStderr)
							);
							if (isKnownSshInfo) {
								logger.debug(
									'[ProcessManager] Suppressing known SSH info message',
									'ProcessManager',
									{
										sessionId,
				...
				childProcess.on('exit', code => {
					...
					logger.debug(
						'[ProcessManager] Child process exit event',
						'ProcessManager',
						{
							sessionId,
							code,
					...
					if (sessionId.includes('group-chat-')) {
						console.log(
							`[GroupChat:Debug:ProcessManager] EXIT for session ${sessionId}`
						);
						console.log(`[GroupChat:Debug:ProcessManager] Exit code: ${code}`);
						console.log(
							`[GroupChat:Debug:ProcessManager] isStreamJsonMode: ${isStreamJsonMode}`
						);
						console.log(
							`[GroupChat:Debug:ProcessManager] isBatchMode: ${isBatchMode}`
						);
						console.log(
							`[GroupChat:Debug:ProcessManager] resultEmitted: ${managedProcess.resultEmitted}`
						);
						console.log(
							`[GroupChat:Debug:ProcessManager] streamedText length: ${
								managedProcess.streamedText?.length || 0
							}`
						);
						console.log(
							`[GroupChat:Debug:ProcessManager] jsonBuffer length: ${
								managedProcess.jsonBuffer?.length || 0
							}`
						);
						console.log(
							`[GroupChat:Debug:ProcessManager] stderrBuffer length: ${
								managedProcess.stderrBuffer?.length || 0
							}`
						);
						console.log(
							`[GroupChat:Debug:ProcessManager] stderrBuffer preview: "${(
								managedProcess.stderrBuffer || ''
							).substring(0, 500)}"`
						);
					}
					...
					if (outputParser && !managedProcess.errorEmitted) {
						...
						if (agentError) {
							...
							agentError.sessionId = sessionId;
							logger.debug(
								'[ProcessManager] Error detected from exit',
								'ProcessManager',
								{
									sessionId,
									exitCode: code,
					...
					if (
						...
						if (sshError) {
							...
							logger.debug(
								'[ProcessManager] SSH error detected at exit',
								'ProcessManager',
								{
									sessionId,
									exitCode: code,
					...
					if (
						...
						managedProcess.tempImageFiles.length > 0
					) {
					...
					if (isBatchMode && managedProcess.querySource) {
						...
						logger.debug(
							'[ProcessManager] Query complete event emitted',
							'ProcessManager',
							{
								sessionId,
								duration,
				...
				childProcess.on('error', error => {
					logger.error(
						'[ProcessManager] Child process error',
						'ProcessManager',
				...
				if (isStreamJsonMode && prompt && images) {
					...
					logger.debug(
						'[ProcessManager] Sending stream-json message with images',
						'ProcessManager',
						{
							sessionId,
					...
					logger.debug(
						'[ProcessManager] Closing stdin for batch mode',
						'ProcessManager',
						{ sessionId }
					);
	...
	/**
	 * Buffer data and emit in batches to reduce IPC event frequency.
	 * Data is accumulated and flushed every 50ms or when the buffer exceeds 8KB.
	 */
	private emitDataBuffered(sessionId: string, data: string): void {
		const managedProcess = this.processes.get(sessionId);
		if (!managedProcess) {
			// Process already exited, emit immediately
		...
		// Flush immediately if buffer is large (keeps latency reasonable for big chunks)
		if (managedProcess.dataBuffer.length > 8192) {
			this.flushDataBuffer(sessionId);
			return;
		}
	...
	private flushDataBuffer(sessionId: string): void {
		const managedProcess = this.processes.get(sessionId);
	...
	write(sessionId: string, data: string): boolean {
		...
		try {
			if (process.isTerminal && process.ptyProcess) {
				logger.debug(
					'[ProcessManager] Writing to PTY process',
					'ProcessManager',
				...
				logger.debug(
					'[ProcessManager] Writing to child process stdin',
					'ProcessManager',
					{ sessionId, pid: process.pid }
				);
			...
			logger.error(
				'[ProcessManager] No valid input stream for session',
				'ProcessManager',
				{ sessionId }
			);
			return false;
	...
	interrupt(sessionId: string): boolean {
		...
		if (!process) {
			logger.error(
				'[ProcessManager] interrupt() - No process found for session',
				'ProcessManager',
				{ sessionId }
			);
			return false;
		...
		try {
			if (process.isTerminal && process.ptyProcess) {
				...
				logger.debug(
					'[ProcessManager] Sending Ctrl+C to PTY process',
					'ProcessManager',
					{ sessionId, pid: process.pid }
				);
				...
				logger.debug(
					'[ProcessManager] Sending SIGINT to child process',
					'ProcessManager',
					{ sessionId, pid: process.pid }
				);
			...
			logger.error(
				'[ProcessManager] No valid process to interrupt for session',
				'ProcessManager',
				{ sessionId }
			);
			return false;
	...
	runCommand(
		...
		return new Promise(resolve => {
			...
			childProcess.stdout?.on('data', (data: Buffer) => {
				...
				logger.debug(
					'[ProcessManager] runCommand stdout RAW',
					'ProcessManager',
					{
						sessionId,
				...
				logger.debug(
					'[ProcessManager] runCommand stdout FILTERED',
					'ProcessManager',
					{
						sessionId,
				...
				if (output.trim()) {
					...
					logger.debug(
						'[ProcessManager] runCommand EMITTING data event',
						'ProcessManager',
						{ sessionId, outputLength: output.length }
					);
	...
	private async runCommandViaSsh(
		...
		if (envExports) {
			remoteCommand = `cd '${remoteCwd.replace(
				/'/g,
				"'\\''"
			)}' && ${envExports} $SHELL -lc "${escapedCommand}"`;
		} else {
			remoteCommand = `cd '${remoteCwd.replace(
				/'/g,
				"'\\''"
			)}' && $SHELL -lc "${escapedCommand}"`;
		}

		// Wrap the entire thing for SSH: use double quotes so $SHELL expands on remote
		const wrappedForSsh = `$SHELL -c "${shellEscapeForDoubleQuotes(
			remoteCommand
		)}"`;
		sshArgs.push(wrappedForSsh);

		logger.info(
			'[ProcessManager] runCommandViaSsh spawning',
			'ProcessManager',
			{
				sessionId,
		...
		childProcess.stdout?.on('data', (data: Buffer) => {
			...
			if (output.trim()) {
				logger.debug(
					'[ProcessManager] runCommandViaSsh stdout',
					'ProcessManager',
					{ sessionId, length: output.length }
				);
				this.emit('data', sessionId, output);
			}
		});
		...
		childProcess.stderr?.on('data', (data: Buffer) => {
			const output = data.toString();
			logger.debug(
				'[ProcessManager] runCommandViaSsh stderr',
				'ProcessManager',
				{ sessionId, output: output.substring(0, 200) }
			);
