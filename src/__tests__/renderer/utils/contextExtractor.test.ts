...
describe('findDuplicateContent', () => {
  it('should detect exact duplicate log entries', () => {
    const longText = 'This is a longer message that exceeds the minimum length for duplicate detection. '.repeat(3);
    ...
    const contexts: ContextSource[] = [
      {
        ...
        projectRoot: '/project',
        name: 'Context 1',
        logs: [createMockLog({ text: 'Unique message one that is long enough to be considered for deduplication purposes' })],
        agentType: 'claude-code',
      },
      {
        type: 'tab',
        ...
        projectRoot: '/project',
        name: 'Context 2',
        logs: [createMockLog({ text: 'Different unique message two that is also long enough for deduplication consideration' })],
        agentType: 'claude-code',
      },
    ];
