...
function generateLargeDataset(numDays: number): StatsAggregation {
  ...
  return {
    ...
    byAgent: {
      'claude-code': { count: Math.floor(totalQueries * 0.6), duration: Math.floor(totalDuration * 0.6) },
      'aider': { count: Math.floor(totalQueries * 0.3), duration: Math.floor(totalDuration * 0.3) },
      'opencode': { count: Math.floor(totalQueries * 0.1), duration: Math.floor(totalDuration * 0.1) },
    },
    bySource: {
...
function generateManyAgentsData(numAgents: number): StatsAggregation {
  ...
  const agentNames = [
    'claude-code', 'aider', 'opencode', 'gpt-engineer', 'cursor',
    'codeium', 'copilot', 'tabnine', 'amazon-q', 'gemini-cli',
    'qwen-coder', 'deepseek', 'codellama', 'starcoder', 'phind',
  ];
  ...
  return {
    ...
    byDay: [{ date: new Date().toISOString().split('T')[0], count: totalQueries, duration: totalDuration }],
  };
}
...
describe('Chart Performance: Recharts NOT Used (Custom SVG Charts)', () => {
  describe('Architecture verification', () => {
    ...
    it('ActivityHeatmap uses custom divs, not Recharts', () => {
      ...
      const { container } = render(
        <ActivityHeatmap data={data} timeRange="month" theme={theme} />
      );
  ...
  describe('Large dataset handling (365 days)', () => {
    ...
    it('DurationTrendsChart X-axis labels are intelligently reduced for year view', () => {
      ...
      const xAxisLabels = Array.from(textElements).filter(
        el => !el.textContent?.match(/^\d+(s|m|h)$/) && // Not Y-axis labels
  ...
  describe('DOM element count optimization', () => {
    ...
    it('AgentComparisonChart scales linearly with agent count', () => {
      ...
      const { container: container3 } = render(
        <AgentComparisonChart data={data3} theme={theme} />
      );
  ...
  describe('Performance with edge cases', () => {
    it('handles empty data without errors', () => {
      ...
      expect(() => render(
        <DurationTrendsChart data={emptyData} timeRange="year" theme={theme} />
      )).not.toThrow();

      expect(() => render(
        <ActivityHeatmap data={emptyData} timeRange="year" theme={theme} />
      )).not.toThrow();
      ...
      expect(() => render(
        <SourceDistributionChart data={emptyData} theme={theme} />
      )).not.toThrow();
    });
    ...
    it('handles very large duration values without overflow', () => {
      const largeValues: StatsAggregation = {
        ...
        byDay: [{ date: new Date().toISOString().split('T')[0], count: 100000, duration: Number.MAX_SAFE_INTEGER / 2 }],
      };

      expect(() => render(
        <DurationTrendsChart data={largeValues} timeRange="day" theme={theme} />
      )).not.toThrow();
    });
    ...
    it('handles many agents (15+) without performance degradation', () => {
      ...
      const { container } = render(
        <AgentComparisonChart data={manyAgents} theme={theme} />
      );
