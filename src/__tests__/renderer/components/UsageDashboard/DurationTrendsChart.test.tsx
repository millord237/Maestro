  ...
  describe('Grid and Axes', () => {
    ...
    it('renders Y-axis tick labels', () => {
      ...
      const hasYLabel = Array.from(textElements).some(
        (el) => el.textContent?.match(/^\d+(s|m|h)$/)
      );
    ...
    it('renders X-axis date labels', () => {
      ...
      const textElements = container.querySelectorAll('text');
      const hasDateLabels = Array.from(textElements).some(
        (el) => el.textContent?.match(/^(Mon|Tue|Wed|Thu|Fri|Sat|Sun)$/)
      );
  ...
  describe('Data Point Interaction', () => {
    it('enlarges data point on hover', () => {
      ...
      if (circles.length > 0) {
        ...
        expect(parseInt(hoveredRadius || '0')).toBeGreaterThan(
          parseInt(initialRadius || '0')
        );
      }
    });
  });
