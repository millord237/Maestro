...
export function QuickActionsModal(props: QuickActionsModalProps) {
  ...
  const mainActions: QuickAction[] = [
    ...
    ...(isAiMode && canSummarizeActiveTab && onSummarizeAndContinue ? [{ id: 'summarizeAndContinue', label: 'Summarize Context', shortcut: tabShortcuts?.summarizeAndContinue, subtext: 'Compact context into a fresh tab', action: () => { onSummarizeAndContinue(); setQuickActionOpen(false); } }] : []),
    ...(activeSession && hasActiveSessionCapability?.('supportsContextMerge') && onOpenMergeSession ? [{ id: 'mergeSession', label: 'Merge Contexts', shortcut: shortcuts.mergeSession, subtext: 'Combine contexts from multiple sessions', action: () => { onOpenMergeSession(); setQuickActionOpen(false); } }] : []),
    ...(activeSession && hasActiveSessionCapability?.('supportsContextMerge') && onOpenSendToAgent ? [{ id: 'sendToAgent', label: 'Send Context to Agent', shortcut: shortcuts.sendToAgent, subtext: 'Transfer context to a different AI agent', action: () => { onOpenSendToAgent(); setQuickActionOpen(false); } }] : []),
