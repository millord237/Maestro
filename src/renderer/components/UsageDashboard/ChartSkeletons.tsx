      ...
      <div className="space-y-2">
        {[85, 60, 45, 30].map((width, i) => (
          <div key={i} className="flex items-center gap-3" style={{ height: 28 }}>
            ...
            <SkeletonBox
              theme={theme}
              style={{ width: 112, height: 14, flexShrink: 0 }}
            />
            {/* Bar */}
            <div className="flex-1">
              <SkeletonBox
                theme={theme}
                style={{ width: `${width}%`, height: 28 }}
              />
            </div>
            {/* Value */}
            <SkeletonBox
              theme={theme}
              style={{ width: 48, height: 14, flexShrink: 0 }}
            />
          </div>
        ))}
      </div>
    ...
    >
      {Array.from({ length: 6 }).map((_, i) => (
        ...
        >
          ...
          <SkeletonBox
            theme={theme}
            style={{ width: '70%', height: 12, marginBottom: 8 }}
          />
          {/* Value */}
          <SkeletonBox
            theme={theme}
...
export function DashboardSkeleton({
  ...
  return (
    <div className="space-y-6" data-testid="dashboard-skeleton">
      ...
      {viewMode === 'autorun' && (
        <AutoRunStatsSkeleton theme={theme} columns={autoRunStatsCols} />
      )}
    </div>
  );
}
