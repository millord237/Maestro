...
export function SourceDistributionChart({
  ...
  const sourceData = useMemo((): SourceData[] => {
    const interactiveValue =
      metricMode === 'count' ? data.bySource.user : (data.bySource.user / (data.bySource.user + data.bySource.auto || 1)) * data.totalDuration;
    const autoValue =
      metricMode === 'count' ? data.bySource.auto : (data.bySource.auto / (data.bySource.user + data.bySource.auto || 1)) * data.totalDuration;
