...
export function DurationTrendsChart({ data, timeRange, theme }: DurationTrendsChartProps) {
  ...
  const chartData = useMemo((): DataPoint[] => {
    ...
    const rawDurations = data.byDay.map((day) =>
      day.count > 0 ? day.duration / day.count : 0
    );
  ...
  return (
    ...
    >
        ...
        {hoveredPoint && tooltipPos && (
          ...
          >
            ...
            <div style={{ color: theme.colors.textDim }}>
              <div>
                Avg Duration: <span style={{ color: theme.colors.textMain }}>{formatDuration(hoveredPoint.displayDuration)}</span>
              </div>
              {showSmoothed && hoveredPoint.rawDuration !== hoveredPoint.smoothedDuration && (
                <div>
                  Raw: <span style={{ color: theme.colors.textMain }}>{formatDuration(hoveredPoint.rawDuration)}</span>
                </div>
              )}
              <div>
      ...
      {/* Legend */}
      <div className="flex items-center justify-end gap-4 mt-3 pt-3 border-t" style={{ borderColor: theme.colors.border }}>
