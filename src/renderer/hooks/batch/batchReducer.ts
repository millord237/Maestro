...
function validateAndTransition(
  ...
  switch (eventType) {
    case 'START_BATCH':
      ...
      event = { type: 'START_BATCH', payload: { sessionId, documents: [], totalTasks: 0, loopEnabled: false, maxLoops: null, worktreeActive: false, worktreePath: null, worktreeBranch: null } };
      break;
    case 'TASK_COMPLETED':
    ...
    case 'LOOP_COMPLETED':
      ...
      break;
    case 'ERROR_OCCURRED':
      event = { type: 'ERROR_OCCURRED', payload: { error: { type: 'unknown', message: '', recoverable: false, agentId: '', timestamp: 0 }, documentIndex: 0 } };
      break;
    default:
  ...
  switch (action.type) {
    ...
    case 'COMPLETE_BATCH': {
      ...
      if (currentState?.processingState === 'COMPLETING') {
        const result = validateAndTransition(sessionId, currentState.processingState, 'BATCH_FINALIZED');
