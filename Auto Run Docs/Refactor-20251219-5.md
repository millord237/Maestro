...
- [x] **Task 100:** `src/renderer/hooks/useModalManager.ts`
  ...
  **Audit Result:** ⚠️ Dead code removed. The entire `useModalManager` hook was dead code - never imported by application code (only referenced by its own tests and ARCHITECTURE.md). The codebase uses `useLayerStack` + `LayerStackContext` for modal management, which provides a more sophisticated API (escape key delegation, modal vs overlay distinction, `onBeforeClose` for confirmations, debug APIs, focus trap support). Removed `useModalManager.ts` (136 lines) and its test file `useModalManager.test.ts` (686 lines). Also removed stale ARCHITECTURE.md references to both `useModalManager` and `useFocusManager` (previously removed in Task 98). Build succeeds; all relevant tests pass.

- [x] **Task 101:** `src/renderer/hooks/usePlaybookManagement.ts`
  ...
  **Audit Result:** ⚠️ Type duplication fixed + API types aligned. Found and fixed:
  1. **Type duplication in hook**: `onApplyPlaybook` callback parameter was an inline duplicate of `PlaybookConfigState` interface - consolidated to use `PlaybookConfigState`
  2. **Dead import**: Removed unused `PlaybookDocumentEntry` import
  3. **API type mismatch**: `preload.ts` and `global.d.ts` playbook API types were missing `maxLoops`, `worktreeSettings.prTargetBranch`, and `updatedAt` fields that the hook code actually uses - added missing fields to both type definition locations
  4. **Dead React import**: Fixed `PlaybookDeleteConfirmModal.tsx` importing React but only using `useRef`

  All exports are actively used by `BatchRunnerModal.tsx`. Hook follows correct patterns: `useClickOutside` for dropdown, `useMemo` for modification detection, `useCallback` for handlers. Error handling uses appropriate `console.error` for async operations. All 300 playbook and BatchRunnerModal tests pass.

- [x] **Task 102:** `src/renderer/hooks/useRemoteIntegration.ts`
  ...
  **Audit Result:** ⚠️ Type mismatch fixed + debug logging removed. Found and fixed:
  1. **Type mismatch**: `onRemoteSelectSession` in `global.d.ts` was typed as `(sessionId: string) => void` but preload.ts implementation accepts optional `tabId` parameter - fixed to `(sessionId: string, tabId?: string) => void`
  2. **Debug logging**: Removed 25+ `console.log` debug statements throughout the hook (setup messages, command/mode/interrupt/selection logging) - kept only `console.error` for actual error cases

  All 7 listener effects are actively used: `onRemoteCommand`, `onRemoteSwitchMode`, `onRemoteInterrupt`, `onRemoteSelectSession`, `onRemoteSelectTab`, `onRemoteNewTab`, `onRemoteCloseTab`, `onRemoteRenameTab`. No dead code paths. The last useEffect (tab broadcasting) intentionally has no dependency array - uses ref-based change detection to broadcast only when tab state changes. Well-typed interfaces (`UseRemoteIntegrationDeps`, `UseRemoteIntegrationReturn`). Added comprehensive test suite (22 tests). Build succeeds.

- [x] **Task 103:** `src/renderer/hooks/useWebBroadcasting.ts`
  ...
  **Audit Result:** ⚠️ Type duplication fixed + debug logging removed. Found and fixed:
  1. **Type duplication**: `RightPanelHistoryHandle` was a redundant subset of `RightPanelHandle` from `RightPanel.tsx`. Removed the duplicate interface and updated the hook to import and use `RightPanelHandle` directly. Removed the dead `RightPanelHistoryHandle` export from `hooks/index.ts`.
  2. **Debug logging**: Removed `console.log` statement from external change handler (line 39).

  Hook is otherwise clean: single useEffect subscribes to `window.maestro.history.onExternalChange`, reloads history from disk to bypass electron-store cache, and refreshes the history panel. Properly cleans up subscription on unmount. Added comprehensive test suite (6 tests) covering subscription/unsubscription, external change handling, null ref safety, and ref change behavior. Build succeeds.

- [x] **Task 104:** `src/renderer/hooks/useWorktreeValidation.ts`
  ...
  **Audit Result:** ⚠️ Type location fixed + tests added. Found and fixed:
  1. **Type smell**: `WorktreeValidationState` and `GhCliStatus` were defined in the component `GitWorktreeSection.tsx` but imported by the hook - reverse of typical pattern. Moved both interfaces to `src/renderer/types/index.ts` (alongside related `WorktreeConfig`) and updated hook + component to import from central types location. Component re-exports types for backward compatibility.
  2. **No tests**: Added comprehensive test suite (12 tests) covering initial state, path validation, branch mismatch detection, different repository detection, error handling, and debouncing behavior.

  Hook is otherwise clean: well-documented with JSDoc, proper debouncing (500ms), correct error handling for all async paths, clean useEffect cleanup. Uses proper IPC calls (`worktreeInfo`, `getRepoRoot`, `status`) from `window.maestro.git`. All tests pass. Build succeeds.
